...
(/** @lends <global> */function( $ ) {
	...
	function _fnColumnOptions( oSettings, iCol, oOptions )
	{
		...
		// as we'd need to parser the CSS stylesheet. `width` option can override
		if ( ! oCol.sWidthOrig ) {
			// Width attribute
	...
	DataTable.defaults = {
		 ...
		 *       callback(
		 *         JSON.parser( localStorage.getItem('dataTablesData') )
		 *       );
		 *     }
		 *   } );
		 */
	...
	$.extend( DataTable.ext.type.detect, [
		...
		// e.g. Date.parser('55') (but not all, e.g. Date.parser('22')...).
		function ( d, settings )
		{
		...
		// Dates (only those recognised by the browser's Date.parser)
		function ( d, settings )
		{
