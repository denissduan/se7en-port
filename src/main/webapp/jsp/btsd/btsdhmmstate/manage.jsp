...
<div class="page-header">
	<form id="searchForm" action="btsdHmmState/query.do" method="post" class="form-inline" role="form">
		<div class="form-group">
			<label class="control-label" for="type">模型类型1.日线2.时线3.五分线4.历史交易</label>
			<input type="text" id="type" name="type" class="input-mini" placeholder="" />
			<script type="text/javascript">
				$('#type').ace_spinner({value:null,min:1 ,max: 256,step:1, btn_up_class:'btn-info' , btn_down_class:'btn-info'});
			</script>
		</div>
		<div class="form-group">
...
<script type="text/javascript">
	(function(){
		...
		$('#newBtn').click(function(){
			loadMain('btsdHmmState/detail.do');
		})
		
		/*编辑虚拟币编码*/
		$('#editBtn').click(function(){
			if($.core.grids[0].checkSelect() == true){
				loadMain('btsdHmmState/detail.do?id=' + $.core.grids[0].curId);
			}
		})
		...
		$('#delBtn').click(function(){
			if($.core.grids[0].checkSelect() == true){
				confirm("确认删除?",function(){
					ajaxGet("btsdHmmState/delete.do?id="+$.core.grids[0].curId,function(data){
						if(data.sign == true){
							alert("删除成功");
							refreshPage();
						}else{
